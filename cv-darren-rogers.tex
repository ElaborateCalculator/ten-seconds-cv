...
% Hard skill bars
\programmingNoLOC{{JavaScript  $\textbullet$ Java / 2}, {Bash $\textbullet$ Python / 5}, {  Perl $\textbullet$ Groovy (Jenkins DSL) $\textbullet$ SQL / 4}}
...
\hardskills{{Linux $\textbullet$ UNIX (AIX) / 6}, {Db2 $\textbullet$ Postgresql $\textbullet$ MySQL / 6}, {DNS $\textbullet$ DHCP $\textbullet$ VPN $\textbullet$ Kerberos / 4}, {LDAP / 5}, {Git $\textbullet$ GitHub / 6}, {IBM Cloud $\textbullet$ Azure / 5}, {Jenkins / 4}, {Kubernetes $\textbullet$ Docker $\textbullet$ Podman / 5}}


% Projects text
\projects{
    \textbf{VLDB (Very Large Database)} - Helped design, construct and maintain Guinness World Record-winning data warehouse \\
    \textbf{GDPC (Geographically Dispersed Purescale Cluster)} - Designed and built test instance where database nodes and storage were separated at multi-km distances, in a single pair of racks in a datacentre \\
    \textbf{Cloud Migration} - Led migration of existing VM-based offerring to IBM Cloud, including design and implementation of safe and secure customer data migration from on-prem database to cloud service
}
